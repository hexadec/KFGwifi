 ...
 * The strength of the algorithms is not tested! The strongest one is <i>Threedog</i> (which uses {@link ThreedogDouble} and {@link ThreedogTreble})
...
public class EncryptUtils {
    ...
    private int BLOCK_SIZE = 96;
    private int BSBYTE = BLOCK_SIZE/8;

    public  String base64encode(String text){
        try {
            if (text.equals(null))throw new Exception();
            String rez = Base64.encodeToString(text.getBytes( ENCODING ), Base64.DEFAULT);
            return rez;
        }
        catch ( Exception e ) {
            return null;
    ...
    public String cryptThreedog(String data, boolean decrypt,String key)  {
        ...
        if (decrypt) {
            data = cryptThreedogDouble(data,decrypt,key);
        }
        if (!decrypt) {
        ...
        for(int p=0; p<nParts; p++) {
            ...
            if(!decrypt) partByte = stringToByteArray(encryptSTEP11(byteArrayToString(partByte),key));
            if(decrypt) partByte = stringToByteArray(decryptSTEP11(byteArrayToString(partByte), key));
            if (decrypt){
                for(int i=0;i<BSBYTE;i++){
                    partByte[i]=(byte)((partByte[i]^(byte)((i+pkl)*(p/(i+1)))));
        ...
        if (!decrypt){
            return cryptThreedogDouble(byteArrayToString(res), decrypt,key);
        }
        else{
    ...
    public String cryptThreedogDouble(String data, boolean decrypt,String key)  {
        if (data.equals(null)||key.equals(null)) return null;
        if (decrypt) {
            data = cryptThreedogTreble(data,decrypt,key);
        }
        ...
        String partStr = "";
        byte[] partByte = new byte[BSBYTED];
        byte[] keyb = stringToByteArray(key);
        int kl = keyb.length;
        for(int p=0; p<nParts; p++) {
            ...
            if (!decrypt){
                ...
                if(!decrypt) partByte = stringToByteArray(encryptSTEP13X(byteArrayToString(partByte),key));
                if(decrypt) partByte = stringToByteArray(decryptSTEP13X(byteArrayToString(partByte),key));
        ...
        if (!decrypt){
            return cryptThreedogTreble(byteArrayToString(res), decrypt, key);
        }
        else{
    ...
    public String cryptThreedogTreble(String data, boolean decrypt,String key)  {
        ...
        for(int p=0; p<nParts; p++) {
            ...
            if(!decrypt) partByte = stringToByteArray(encryptSTEP17Y(byteArrayToString(partByte),key));
            if(decrypt) partByte = stringToByteArray(decryptSTEP17Y(byteArrayToString(partByte),key));
            if (decrypt){
                for(int i=0;i<BSBYTE;i++){
                    partByte[i]=(byte)((partByte[i]^((byte)((i+p)*((2*p)+1/(i+1)))^i)));
